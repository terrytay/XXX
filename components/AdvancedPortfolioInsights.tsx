...
export default function AdvancedPortfolioInsights({
  clients,
  showPrivateInfo,
}: Props) {
  ...
  const insights = useMemo(() => {
    ...
    const cashFlowAnalysis = {
      totalCashFlow: clients.reduce(
        (sum, client) => sum + client.tiv - client.tia,
        0
      ),
      ...
      dividendIncome: clients.reduce(
        (sum, client) => sum + client.totalDividendsPaidout,
        0
      ),
    };
    ...
    const performanceDistribution = {
      excellent: clients.filter(
        (c) => parseFloat(c.grossProfit.replace("%", "")) > 20
      ).length,
      ...
      poor: clients.filter(
        (c) => parseFloat(c.grossProfit.replace("%", "")) <= 0
      ).length,
    };
    ...
    const durationAnalysis = {
      ...
      youngPolicies: clients.filter((c) => c.duration >= 1 && c.duration < 3)
        .length,
      maturePolicies: clients.filter((c) => c.duration >= 3 && c.duration < 10)
        .length,
      veteranPolicies: clients.filter((c) => c.duration >= 10).length,
    };

    // Top and bottom performers
    const sortedByPerformance = [...clients].sort(
      (a, b) =>
    ...
    const cashAnalysis = {
      ...
      averageCashPercentage:
        clients.length > 0
          ? clients.reduce((sum, c) => sum + (c.cash / c.tiv) * 100, 0) /
            clients.length
          : 0,
    };
    ...
    clients.forEach((client) => {
      client.fundAllocations.forEach((fund) => {
        ...
        fundTypeAnalysis[fundType] =
          (fundTypeAnalysis[fundType] || 0) + fund.value;
      });
    });
    ...
    const efficiencyMetrics = {
      cashDragEffect:
        (cashAnalysis.totalCashReserves / cashFlowAnalysis.totalValue) * 100,
      averagePerformance:
        clients.length > 0
          ? clients.reduce(
              (sum, c) => sum + parseFloat(c.grossProfit.replace("%", "")),
              0
            ) / clients.length
          : 0,
      underperformingCount: clients.filter(
        (c) => parseFloat(c.grossProfit.replace("%", "")) < 5
      ).length,
    };
                ...
                <ScatterChart data={performanceVsRiskData}>
                  ...
                  <Tooltip
                    ...
                    labelFormatter={(label, payload) =>
                      payload?.[0]
                        ? `Client: ${
                            showPrivateInfo ? payload[0].payload.name : "***"
                          }`
                        : ""
                    }
                  />
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Progress
                    value={
                      (insights.performanceDistribution.good / clients.length) *
                      100
                    }
                    className="w-20"
                  />
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Progress
                    value={
                      (insights.performanceDistribution.poor / clients.length) *
                      100
                    }
                    className="w-20"
                  />
            ...
            <div className="mt-4 p-3 bg-muted/50 rounded-lg">
              <p className="text-xs text-muted-foreground">
                ...
                out of {clients.length} policies are performing above 10%
                returns, representing strong portfolio health.
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Total cash across{" "}
                  {insights.cashAnalysis.clientsWithCash.length} policies
                </p>
              </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-orange-600 mt-1">
                        {insights.cashAnalysis.clientsWithCash.length} policies
                        holding $
                        {format2dp(insights.cashAnalysis.totalCashReserves)} in
                        cash
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-red-600 mt-1">
                        {insights.efficiencyMetrics.underperformingCount}{" "}
                        policies below 5% performance threshold
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      {insights.durationAnalysis.maturePolicies +
                        insights.durationAnalysis.veteranPolicies}
