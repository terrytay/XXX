...
export default function InteractivePortfolioAnalytics({
  clients,
  showPrivateInfo,
  aggregatedData,
}: Props) {
  ...
  const analytics = useMemo(() => {
    ...
    const totalExcessCash = excessiveCashClients.reduce(
      (sum, c) => sum + (c.cash - c.tiv * 0.05),
      0
    );
    ...
    // Risk Analysis
    const highRiskHighCash = clients.filter(
      (c) => c.riskScore > 0.7 && c.cash > c.tiv * 0.1
    );
    const conservativeUnderperformers = clients.filter(
      (c) =>
        c.riskScore < 0.3 &&
        parseFloat(c.grossProfit.replace("%", "")) < 1 &&
        c.duration > 1
    );
    ...
    const performanceDistribution = [
      ...
      },
      {
        ...
        count: clients.filter((c) => {
          const p = parseFloat(c.grossProfit.replace("%", ""));
          return p >= -5 && p < 0;
        }).length,
        color: "#f97316",
      },
      ...
      },
      {
        range: "Above 10%",
        count: clients.filter(
          (c) => parseFloat(c.grossProfit.replace("%", "")) >= 10
        ).length,
        color: "#16a34a",
      },
    ];
    ...
    const totalCashDrag = clients.reduce(
      (sum, c) => sum + Math.max(0, c.cash - c.tiv * 0.05),
      0
    );
    ...
    switch (selectedInsight) {
      ...
      case "underperforming":
        return {
          ...
          action:
            "Review fund allocation and consider switching to higher-performing funds",
        };
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-900">
              {analytics.excessiveCashClients.length}
            </div>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-900">
              {analytics.underperformingClients.length}
            </div>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-900">
              {analytics.switchOpportunities.length}
            </div>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-900">
              {analytics.actionMatrix.critical.length}
            </div>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              <h4 className="font-medium text-sm">
                Cash Efficiency by Client Type:
              </h4>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-medium text-red-800">
                  Critical ({analytics.actionMatrix.critical.length})
                </h4>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-medium text-orange-800">
                  Urgent ({analytics.actionMatrix.urgent.length})
                </h4>
              </div>
              <div className="space-y-2 max-h-32 overflow-y-auto">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-medium text-green-800">
                  Monitor ({analytics.actionMatrix.monitor.length})
                </h4>
              </div>
              <div className="space-y-2 max-h-32 overflow-y-auto">
                    ...
                    <TableCell>
                      <span
                        className={`font-medium ${
                          parseFloat(client.grossProfit.replace("%", "")) > 0
                            ? "text-green-600"
                            : "text-red-600"
                        }`}
                      >
